...
The package includes two implementations of fit of linear mixed models
(LMMs): plain R, and C++ (using [Rcpp]() and the [RcppEigen]()
packages, the latter being an interface to the [Eigen]() C++ linear
algebra library).
...
`TRUE`, the 0 and 1 boundaries are checked explicitly, in seeking to
